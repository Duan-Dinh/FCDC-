...
public interface Daily_ReportRepository extends JpaRepository<Daily_Report,Long> {
    ...
    Daily_Report findDaily_ReportByDateTimeAndUserId(Date date, Long userId);
    //get village
    @Query(value = "SELECT d FROM Daily_Report d where user_id = ?1 ORDER BY id  desc ")
...
//AllSentReportOnedate
 ...
 @Query( value = "SELECT d.* FROM fcdc.daily_report d,user u where u.id = d.user_id and d.date_time like ?1 and u.is_active = 1 and u.village_id = ?2 and u.fullname like %?3% order by id DESC ", nativeQuery = true)
